	...
	else
	{
		...
		foreach ($arResult['ORDERS'] as $key => $order)
		{
            ...
            <div class="lk-navigation">
                <a class="lk-navigation__item active">История заказов</a>
                <a class="lk-navigation__item" href="/personal/">Персональные данные</a>
            </div>
            <div class="lk-orders">
                <div class="lk-orders-row lk-orders-row_header lk-orders-row_header--desktop">
                    <div class="lk-orders-col">Номер заказа</div>
                    <div class="lk-orders-col">Дата</div>
                    <div class="lk-orders-col">Наименование</div>
                    <div class="lk-orders-col">Сумма</div>
                    <div class="lk-orders-col">Скидка</div>
                    <div class="lk-orders-col">Количество</div>
                </div>
                <div class="lk-orders-row">
                    <div class="lk-orders-col"><?= htmlspecialcharsbx($order['ORDER']['ACCOUNT_NUMBER'])?></div>
                    <div class="lk-orders-col"><?= $order['ORDER']['DATE_INSERT_FORMATED'] ?></div>
                    <div class="lk-orders-col">
                    <?foreach ($order['BASKET_ITEMS'] as $basketOrder):?>
                        <?=$basketOrder['NAME']?><br>
                    <?endforeach;?>
                    </div>
                    <div class="lk-orders-col"><?= $order['ORDER']['FORMATED_PRICE'] ?></span></div>
                    <div class="lk-orders-col"><?=$order['ORDER']['DISCOUNT_VALUE']?></span></div>

                    <div class="lk-orders-col"><?= count($order['BASKET_ITEMS']); ?> шт.</div>
                </div>

                <div class="lk-orders__empty">У вас  ещё не было оформленных заказов</div>
            </div>
			<?
		}
	}
